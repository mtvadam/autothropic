...
import type { ChromeTheme, BrowserChromeType } from './devices.js';
...
export function buildSafariModernChrome(hostname: string, theme: ChromeTheme, safeAreaBottom: number, hasBottomNav: boolean, screenCornerRadius: number = 0, pageBackgroundColor?: string): string {
	...
	const overlayPE = hasBottomNav ? '' : 'pointer-events:none;';
	return `<div id="sm-wrap" class="chrome-bar chrome-bottom" data-exp-h="${chromeHeight}" data-col-h="${5 + collapsedH + 20}" data-chrome-h="${chromeHeight}" style="height:${chromeHeight}px;${posStyle};overflow:hidden;${cornerStyle}background:transparent;will-change:height;z-index:2;${overlayPE}transition:height 0.3s cubic-bezier(0.2,0.9,0.3,1);">
	<div id="sm-backing" data-chrome-bg="${defaultChromeBg}" style="padding-top:${SM_TOP_PADDING}px;background:${hasBottomNav ? defaultChromeBg : 'transparent'};position:relative;z-index:1;pointer-events:auto;">
